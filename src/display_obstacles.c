...
{
    if (snowman.positionsnowman.x <= 1920 && snowman.positionsnowman.x > -18*3) {
...
{
    if (flysnowman.positionflysnowman.x <= 1920 && flysnowman.positionflysnowman.x > -34*7) {
        sfSprite_setTexture(flysnowman.s_flysnowman, flysnowman.t_flysnowman, sfTrue);
        sfSprite_setPosition(flysnowman.s_flysnowman, flysnowman.positionflysnowman);
        sfSprite_setTextureRect(flysnowman.s_flysnowman, flysnowman.rectflysnowman);
        sfSprite_setScale(flysnowman.s_flysnowman, flysnowman.resize);
        sfRenderWindow_drawSprite(window, flysnowman.s_flysnowman, NULL);
    }
}

void display_enemie3(sfRenderWindow *window, enemie3_t enemie3)
{
    if (enemie3.positionenemie3.x <= 1920 && enemie3.positionenemie3.x > -18*3) {
    ...
    for (int i = 0; runner->map[i] != '\0'; i++) {
        switch (runner->chainmob[i].countain)
        {
            ...
            case 3 :
                ...
                break;
            case 4 :
                display_gold(window, runner->chainmob[i].gold);
                break;
            case 5 :
                display_flag(window, runner->chainmob[i].flag);
                break;
        }
    }
}